...
class JournalsRepository extends GetxController {
  ...
  static const String USERS_COLLECTIONS = "Users";
  ...
  Future<void> createJournal(JournalModel journal) async {
    try {
      await recordExist(journal.user, journal.createdAt.toDate()) ? throw "Record Already Exists" : await _db.collection(JOURNALS_COLLECTION).add(journal.toJson());
  ...
  Future<List<JournalModel>> getJournalsForUser(String email) async {
    try {
      final userJournalsSnapshot = await _db.collection(JOURNALS_COLLECTION).where("user", isEqualTo: email).orderBy("created_at", descending: true).get();
      if (userJournalsSnapshot.docs.isEmpty) throw 'No journals for user';
      final userJournalsData = userJournalsSnapshot.docs.map((e) => JournalModel.fromSnapshot(e)).toList();
  ...
  /// Fetch User Specific details
  Future<JournalModel> getJournalForUser(String email, DateTime createdAt) async {
    try {
      final userJournalsSnapshot = await _db.collection(JOURNALS_COLLECTION).where("user", isEqualTo: email).orderBy("created_at", descending: true).get();
      if (userJournalsSnapshot.docs.isEmpty) throw 'No journals for user';
      var userJournalsData = userJournalsSnapshot.docs.map((e) => JournalModel.fromSnapshot(e)).toList();

      return  userJournalsData.where((element) => _isSameDate(element.createdAt.toDate(), createdAt)).toList().single;
    } on FirebaseAuthException catch (e) {
      final result = TExceptions.fromCode(e.code);
      throw result.message;
    } on FirebaseException catch (e) {
  ...
  Future<List<JournalModel>> getAllJournals() async {
    try {
      ...
      final journals = snapshot.docs.map((e) => JournalModel.fromSnapshot(e)).toList();
      return journals;
    } on FirebaseAuthException catch (e) {
      final result = TExceptions.fromCode(e.code);
      throw result.message;
  ...
  Future<void> updateUserJournalRecord(String email, String text, String title) async {
    try {
      final userTodayJournalSnapshot = await _db.collection(JOURNALS_COLLECTION).where("user", isEqualTo: email).orderBy("created_at", descending: true).get();
      if (userTodayJournalSnapshot.docs.isEmpty) throw 'No journals for user';
      JournalModel userJournalsData = userTodayJournalSnapshot.docs.map((e) => JournalModel.fromSnapshot(e)).first;
      ...
      if(!_isSameDate(todayDate, journalDate)) {
        ...
        await _db.collection(USERS_COLLECTIONS).add(JournalModel(user: email, text: "", title: "My Journal Today", createdAt: Timestamp.now()).toJson());
        return;
      }
      userJournalsData.text = text;
      userJournalsData.title = title;
      await _db.collection(JOURNALS_COLLECTION).doc(userJournalsData.id).update(userJournalsData.toJson());
    } on FirebaseAuthException catch (e) {
      final result = TExceptions.fromCode(e.code);
      throw result.message;
    } on FirebaseException catch (e) {
  ...
  Future<void> deleteJournal(String email) async {
    try {
      ...
      await _db.collection(JOURNALS_COLLECTION).doc(userJournalsData.id).delete();
    } on FirebaseAuthException catch (e) {
      final result = TExceptions.fromCode(e.code);
      throw result.message;
    } on FirebaseException catch (e) {
